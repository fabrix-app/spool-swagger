 ...
 */
export class SwaggerService extends Service {
...
// Example
  ...
  getInfoVersion(config) {
    ...
    else if (config.tapestries && config.tapestries.prefix) {
      const matches = config.tapestries.prefix.match(/(^|\/)v[0-9.]+($|\/)/)
  ...
  getBasePath(config) {
    if (config.swagger.basePath) {
      ...
      if (config.passport && config.passport.prefix) {
        if (config.tapestries.prefix !== config.passport.prefix) {
          const tapestriesPrefix = config.tapestries.prefix.toLowerCase()
          ...
          let path1 = tapestriesPrefix.length < passportPrefix.length ?
            tapestriesPrefix :
            passportPrefix
          let path2 = tapestriesPrefix.length > passportPrefix.length ?
            tapestriesPrefix :
            passportPrefix
          ...
          standardBasePath = tapestriesPrefix.replace(regExp, '')
      ...
      return config.tapestries.prefix
    }
    else {
      return '/api'
    }
  ...
  getPathDefaultLimit(config) {
    ...
    if (config.tapestries && config.tapestries.models && config.tapestries.models.options &&
        config.tapestries.models.options.defaultLimit) {
      defaultLimit = config.tapestries.models.options.defaultLimit
    }
